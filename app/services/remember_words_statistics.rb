class RememberWordsStatistics
  ...
  class << self
    def call(day: Date.current, days: 365)
      end_day = day.to_date
      date_range = (end_day - (days - 1).days)..end_day
      daily_review_counts = daily_correct_review_counts(date_range)
...

      {
        ...
        daily_review_counts: fill_missing_days(date_range, daily_review_counts),
    ...
    def daily_correct_review_counts(date_range)
      WordQuestionRecord
        .where(
          is_correct: true,
          created_at: date_range.first.in_time_zone.beginning_of_day..date_range.last.in_time_zone.end_of_day
        )
    ...
    def reviews_in_window(day:, days:)
      ...
      WordQuestionRecord.where(is_correct: true, created_at: range).count
    end
    ...
    def active_days_in_window(day:, days:)
      ...
      WordQuestionRecord
        .where(is_correct: true, created_at: range)
        .group("DATE(created_at)")
        .count
        .keys
        .size
